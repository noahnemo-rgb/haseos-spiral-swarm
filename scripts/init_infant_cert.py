#!/usr/bin/env python3
"""Infant cert at HITL promotion — minted by Light-Keeper on the AX-18.
...
Infant WorldSlice must be ⊆ Senior WorldSlice (--senior-cert required).
Optional --queenbee-cert also requires infant ⊆ QueenBee.
"""
...
from haos_dsm_infant import (  # noqa: E402
...
DEFAULT_OUT = "dsm_cert_infant.json"
...
def init_infant_cert(
    *,
    sovereign_id: str,
    senior_cert: dict | None = None,
    ...
    senior = senior_cert
    if senior is None:
        if senior_cert_path is None:
            raise InitError("--senior-cert is required")
        senior = load_cert_json(senior_cert_path, flag="--senior-cert")
    if hosts is None:
    ...
    if qb is None and queenbee_cert_path is not None:
        qb = load_cert_json(queenbee_cert_path, flag="--queenbee-cert")
    if not infant_slice_fits_chain(proposed, senior, qb):
        raise InitError(
            "infant WorldSlice is not ⊆ Senior WorldSlice "
            "(and ⊆ QueenBee when --queenbee-cert is given; "
            "empty infant lists fail closed)"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Mint an Infant HASEOS cert at HITL promotion (Light-Keeper)."
    )
    parser.add_argument(
        "--sovereign-id",
        required=True,
        help="Infant sovereign id (HITL / QueenBee-proposed)",
    )
    parser.add_argument(
        "--senior-cert",
        required=True,
        help="Existing Senior cert JSON; infant slice must be ⊆ that slice",
    )
    parser.add_argument(
        "--out",
        default=DEFAULT_OUT,
    ...
    parser.add_argument(
        ...
        help=f"Must be {DEFAULT_ROLE}; senior is D18",
    )
    parser.add_argument(
        "--host",
        action="append",
    ...
    parser.add_argument(
        "--queenbee-cert",
        default=None,
        help="Optional QueenBee cert JSON; infant slice must also be ⊆ that slice",
    )
    parser.add_argument(
        "--usb-image",
        default=None,
        help="USB-state JSON; writes dsm_cert_infant.json beside it",
    )
    parser.add_argument(
        "--primary-witness",
        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        cert = init_infant_cert(
            sovereign_id=args.sovereign_id,
            senior_cert_path=args.senior_cert,
            out_path=args.out,
            hours=args.hours,
            issuer=args.issuer,
            role=args.role,
    ...
    if args.usb_image:
        print(f"usb cert {INFANT_CERT_FILENAME}", file=sys.stderr)
    return 0
